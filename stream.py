...
한 실행 = 한 파일(열 때 truncate — 이어가기(D79)는 스냅샷 자리까지 자른 뒤 append). 라인 = compact JSON + '\n', 공통 필드 'kind'.
...
class StreamWriter:
    def __init__(self, path, append=False):
        ...
        # append(D79 이어가기): 같은 판의 기록을 이어 쓴다 — 러너가 스냅샷 자리까지 자른 뒤 연다
    ...
    def tell(self):
        """지금까지 쓴 바이트 수(D79 스냅샷 자리) — flush 뒤 바이트 버퍼의 위치(텍스트 tell 의 쿠키가 아니다)."""
        self.f.flush()
        return self.f.buffer.tell()
