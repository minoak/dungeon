...
2. **마을에서 출발한다.** 신전·길드·주점·던전 입구가 있는 마을에서 각자 다른 건물 앞에서 눈을 뜬다. 길은 다 알지만 사람은 같은 구역에
   있어야 보이고 말이 들린다. 길드 게시판의 의뢰를 맡을 수 있고(고블린 소탕·2층 답사·잃어버린 장신구), 접수원·주점 주인·성직자는
   가까이 오면 먼저 말을 걸고(걷던 캐릭터는 그 자리에 멈춰 다음 행동을 고른다), 말을 걸면 답한다. 성직자에게 기도하면 축복의 물약을 한 병 받는다(마시면 공격 능력치가 1 오른다). 번화가와 샛길엔 떠돌이 모험자·견습 모험자·노점 상인이 걸어 다닌다. 준비가 되면
...
```bash
bash _run_gates.sh                                     # 결정론 게이트 70종 일괄 (Git Bash, LLM 0콜, 라이브 데이터와 격리)
...
- 게이트 `verify_*.py` **70종**은 엔진 물리(시야·전투·함정·경로)·스트림 계약·파티/솔로·스캐너·사건층·장비 개체·마을·엔티티 저장소·
