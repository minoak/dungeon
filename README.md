...
2. **마을에서 출발한다.** 번화가·상점가·주거구역·신전·길드·던전 입구의 여섯 구역으로 나뉜 마을에서 각자 다른 건물 앞에서 눈을 뜬다. 길은 다 알지만 사람은 같은 구역에
   있어야 보이고 말이 들린다. 길드 게시판의 의뢰를 맡을 수 있고(고블린 소탕·2층 답사·잃어버린 장신구), 접수원·주점 주인·성직자는
   가까이 오면 먼저 말을 걸고(걷던 캐릭터는 그 자리에 멈춰 다음 행동을 고른다), 말을 걸면 답한다. 성직자에게 기도하면 축복의 물약을 한 병 받는다(마시면 공격 능력치가 1 오른다). 길드 앞마당에는 떠돌이 모험자·견습 모험자가, 번화가에는 노점 상인이 걸어 다닌다. 준비가 되면
   던전 입구에 모여 계단을 내려간다. 건물·사람·구역마다 특징 한 줄이 보이고, 곁에 가면 그곳의 이야기가 들린다.
   새 지도에는 시장·잡화점·대장간·공방·장비점과 등급별 숙소의 공간을 마련했다. 거래·제작·강화·숙박 효과는 아직 없으며 [최초 조감도 기반 마을 v4와 비교 화면](art/town-v4/README.md)에서 확인할 수 있다.
...
5. **귀환과 보고, 또는 전멸.** 게이트를 타면 마을로 돌아온다. 길드 접수원에게 보고하면 원정이 끝난다 — 맡은 의뢰 중 무엇을 채웠고
   무엇을 못 채웠는지가 그 자리에서 갈린다. 죽은 동료는 묘로 남고, 남은 사람들이 결정을 이어간다.
...
론처에서 회사와 모델을 목록으로 고르면 새 원정·이어가기에 적용된다(D80). Gemini 9종·Claude 4종·OpenAI 4종을 제공하며, 목록 밖 모델은 `직접 입력…`으로 지정한다. 로컬에서도 화면에 API 키를 입력할 수 있고, 비워 두면 `.env`의 해당 회사 키를 쓴다.
공개 서버는 화면에 입력한 BYOK 키를 쓴다. 로그인·새 키 연결에서도 회사를 고른다. 입력 키는 파일·브라우저 저장소에 저장하지 않는다.
서버에 운영자 키가 있으면 키를 비워 두고도 한 판을 열 수 있다(D98 — `gemini-3.8-flash` 고정 · 최대 600틱 · 이어가기 없음 · 하루 판 수 상한 · 창을 닫으면 30초 안에 멈춤). 그동안 화면은 모델·키 칸을 숨기고 조건을 서버 값으로 말한다.
판단 가격표는 캐릭터 설정·규칙/장면/기억의 글자 수와 출력 토큰·판단 횟수로 예상 USD 비용을 계산한다. 글자당 토큰 수는 바꿀 수 있는 가정이며 실제 청구액·한 판 총액이 아니다. 현재 파티 설정 길이를 적용하거나 1만 자 등 긴 설정을 가정해 비교할 수 있다.
...
```bash
bash _run_gates.sh                                     # 결정론 게이트 85종 일괄 (Git Bash, LLM 0콜, 라이브 데이터와 격리)
...
- 게이트 `verify_*.py` **85종**은 엔진 물리(시야·전투·함정·경로)·스트림 계약·파티/솔로·스캐너·사건층·장비 개체·마을·엔티티 저장소·
...
무엇이 되고 무엇이 안 되는지는 [docs/STATUS_2026-09-20.md](docs/STATUS_2026-09-20.md) 에 갈라 적었다.
...
판당 50회 시험 제한은 유지 중이며 심사용 긴 판을 열기 전 운영 한도를 정해야 한다.
