...
check('① 정의 로드 — 마을 v3 공간 정의(맵 9·건물 12, 이전 지도 정의 포함) + 동료 프리셋(D81 — 수는 늘어난다)',
      {d['kind'] for d in defs.values()} == set(ENT.KINDS) and len(defs) - len(ENT.by_kind('companion')) == 51 and len(ENT.by_kind('companion')) >= 3
      and len(ENT.by_kind('monster')) == 6 and len(ENT.by_kind('trap')) == 3 and len(ENT.by_kind('object')) == 9 and len(ENT.by_kind('npc')) == 9
