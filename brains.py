...
_CONTEXT_FALLBACK = ("이 글은 성인 모험가들이 등장하는 판타지 역할극 게임의 판단 요청이다. "   # D83(09-18 파트너 "이 게임은 판타지 역할극 게임이 되어야 한다 … 마을은 또 다른 축") — 옛 '던전 게임'
...
def _wire(obs, names=None, compose=False):
    ...
    if obs.get("town"):
        ...
        # D83(09-18 파트너 "승리조건을 두지말자 … 굳이 규칙을 주지 않아도 하고싶은 것 > 하고 싶은 것의 규칙이 따라 붙을거야"):
        #   지침(세계 설명)에서 뺀 원정의 규칙은 원정 중의 관측에 붙는다 — 문장은 옛 지침의 것 그대로. 마을의 관측에는 없다.
        #   계단 모임 규칙은 이미 계단 줄에 붙어 있다(D66 _exit_gather), 보스층의 문은 워프게이트 줄이 말한다(D65).
        L.append("- 여기는 던전이다 — 원정에는 제한 시간이 있다. HP가 0이 되면 이번 원정에서 죽는다.")
    ...
    if ears:
        L.append("- 지금 네 말이 들리는 마을 사람: %s — 응답 JSON 의 `to` 에 그 ID 를 적어 말하면 그 사람에게 건 말이 되고, 그 사람이 말로 답한다. "
                 "마을 사람이 네게 건넨 말을 들은 뒤 `to` 없이 한 잡담도 그 사람에게 한 답으로 들린다. 같은 때 여럿이 한 사람에게 말하면 그 사람은 한 명에게만 답한다. "
                 "한 사람이 되받는 말은 네가 이번에 마을에 머무는 동안 %d번까지다"
