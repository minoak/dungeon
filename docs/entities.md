...
| object·building `use{kind, once?, …}` · object `story{trait, history}` | `interactables.py`(`use_of`·`handle`·`place`) ← `Dungeon._interact` 의 마지막 'nothing' 직전 · `view()` 피처 항목 `use{kind[,heal]}` · 메뉴 줄 머리 · 조합형 대상 태그 | **D89(2026-09-20) 쓰임 부품**: 곁에서 쓰면(use/interact) 무슨 일이 나는가를 정의가 말한다 — 새 오브젝트·새 건물 기능은 JSON 한 장(엔진 무수정). kind 8종: `read`(text 또는 texts[]) · `sit`/`drink`/`warm`(heal, 기본 1) · `browse`(wares[] — 구경만, 매매 없음) · `practice` · `rummage`(loot[{item: potion\|treasure\|nothing, w}] — 세계 시드·자리의 해시로 추첨, 늘 한 번) · `lodge`(HP 전부 + 상태 태그 소거). `once:true` = 한 번 쓰면 끝(피처는 남고 `used_up`). 오브젝트는 피처 type 이 정의의 `type` 과 같은 것(이름까지 같은 정의 먼저), 건물은 `building_defs` 의 정의 id. 엔진이 제 뜻으로 아는 type(chest·fountain·npc …)에는 못 단다. 도감 본문(`knowledge.deep`)을 달면 ③ 지식 해시가 바뀐다 — 오브젝트 소개는 `story` 로. ⚠️이름·글·문장 전부 임시 |
| npc·building `life{<부품 이름>: {덮어쓸 칸}}` · npc `npc.town_life` · object `use.verb` | `entities.comps_of(eid, life)`·`npc(eid, life)` ← `show_runner.build_town`(러너 `DUNGEON_TOWN_LIFE=1` 일 때만) · `interactables.use_of`(`d.town_life`) · 배치 = `art/town-v4` layout 의 `life_objects[{id, entity, cell}]`·`life_npcs[{id, cell, row}]`(`town_layout.compile_layout` 이 검증) | **D90(2026-09-20) 마을 생활**: 끈 판은 옛 판과 바이트가 같아야 하므로 원래 칸은 그대로 두고, 켠 판에서 거짓이 되는 문장('…서비스는 아직 준비 중이다')과 건물의 쓰임을 `life` 에 따로 둔다 — npc 는 문장 칸(line·line_again·role·persona·hail 류)·story, building 은 role·story·use. 판정에 닿는 칸(선물·보고·걸음)은 덮어쓸 수 없다(검증기가 거절). `npc.town_life:true` = 마을 생활 판에서만 나오는 행인(끈 판의 행인 루프가 건너뛴다). `use.verb`(read 전용, 지금은 `look` 하나) = 라벨·태그·문장의 동사만 바꾼다(화단 '살펴보기' — 처리·결과 이름은 read 그대로). 새 오브젝트·새 주민은 JSON 한 장 + layout 한 줄. ⚠️문장 전부 임시 |
