...
> 2026-09-17(D81): 편집 카드는 **1번 칸(내 캐릭터) 하나**다. 저장한 캐릭터는 그 카드로 불러와 고치고, 2·3번 칸에서는 동료로 **고르기만** 한다(준비된 동료 `entities/companion/` 와 같은 목록). 1번 칸에 선 저장본은 동료 후보에서 빠진다. 아래 '원하는 카드/파티 칸' 설명은 이 구성으로 읽는다.
...
브라우저 재접속, 복원, 성격·헤어 유지, 저장 캐릭터를 동료 칸에서 고르기, 모바일 폭, 출발 버튼의 파티 저장을 확인한다(D81 흐름).

![내 캐릭터로 불러오고 저장한 다른 캐릭터를 동료 칸에 세운 검증 화면](character-presets-preview.png)

## 판 기록과 캠페인(D78, 2026-09-16)

파티를 확정할 때(D81부터 출발 버튼이) 론처는 `POST /api/party` 에 `{slots, preset_ids}` 를 보낸다. 준비된 동료 칸은 `{"companion": "<id>"}` 이고 그 자리의 `preset_ids` 는 빈 문자열이다(동료 프리셋은 1회용). `preset_ids` 는 슬롯과 같은 순서의 저장 캐릭터 `id`(불러오기·저장한
