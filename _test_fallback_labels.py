...
import json
import os
os.environ["DUNGEON_ACTION_MODE"] = "menu"
...
check("str 모킹 하위호환(파싱 실패)", r["src"] == "error" and "파싱 실패" in r["reason"], r["reason"])
r = brain_with("아무 JSON 없는 잡담")
check("JSON 없음 -> reason", "JSON 없음" in r["reason"], r["reason"])
r = brain_with('{"choice": 999, "say": "돌격", "reason": "이유"}')
check("무효 choice -> 행동 해석 실패 라벨", r["src"] == "error" and "행동 해석 실패" in r["reason"], r["reason"])
