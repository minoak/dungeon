...
2026-09-20 D90(마을 생활): 그 자리에 쓸 수 있는 오브젝트가 선다. `build.py` 의 `life_objects`(고정물 곁의 보행 바닥 칸 —
id 는 `solids` 의 id 와 같고, 곁 칸인지 빌드 때 검사한다 · 정의는 `entities/object` 의 쓰임 부품)와 `life_npcs`(새 정착 주민 ·
`row` 는 기존 NPC 시트의 행을 빌린 것)가 `layout.json` 에 같은 이름의 필드로 실린다. 엔진은 러너 스위치 `DUNGEON_TOWN_LIFE=1` 을
켠 판에서만 이 필드를 읽는다(끈 판의 마을은 옛 그대로 — `verify_townlife.py`). 서쪽 좌판은 잡화점 건물과 같은 자리라 오브젝트를
따로 세우지 않았고, 분수 곁 화분 둘은 장식으로 남겼다. 제작·강화·거래는 여전히 없다(구경만). 숙박은 값 없는 묵기(회복)다.
