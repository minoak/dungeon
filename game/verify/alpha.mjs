...
try {
  ...
  await page.goto(base + '/launcher/?mode=alpha');
  ...
  check('알파 선택 · 시작 스킬 6개 미리보기 · 마을 비활성',
    await page.inputValue('#runMode') === 'alpha' && await page.locator('#alphaPreview .wl-skill').count() === 6
  ...
  check('일반판 전환 시 알파 설명 숨김 · 마을 복원', await page.isHidden('#alphaInfo') && !await page.isDisabled('#town'));
  await page.selectOption('#runMode', 'alpha');
  ...
  check('실제 러너 설정: 5층 · 600틱 · 알파 3종 ON', await page.evaluate(() => {
    ...
    return m.depths === 5 && m.max_turns === 600 && m.alpha.skills && m.alpha.trpg_combat && m.alpha.random_skill_effective;
  }));
  ...
  check('옛 뷰어에도 알파 표식과 스킬 카드', (await page.textContent('#runInfo')).includes('스킬 알파'));
