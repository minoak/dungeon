...
| `history` | (2026-09-07 D38 개정 2 additive) 최근 판단 장부 여부(bool, `DUNGEON_HISTORY` 기본 1 — brains 표현층, notes 와 같은 급). true 면 결정 때 직전 판단+그 뒤 궤적(trail)을 결정 단위로 접어 두고, 직전보다 앞선 결정 최대 5개를 obs.history → wire '## 네 최근 판단들'로 되돌려준다(한 줄 = 선택 + 그 뒤 일어난 일 꼬리표 체인, 멈춘 이유 포함). 궤적(trail) 판에서만 쌓인다. 스트림 내용 무변(obs 미기록) — 프롬프트 표현 메타 |
