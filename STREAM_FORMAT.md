...
| `npc_hail_stop` | (2026-09-15 D76 additive) NPC 인사에 걸음을 멈추는 판(bool, `DUNGEON_NPC_HAIL_STOP` 러너 기본 1 — npc_hail 판만). true 면 인사를 받은 걷던·기다리던 캐릭터의 order 가 끊기고 그 틱에 결정한다(`tick.npc_hails[].stopped`, 직전 결과 `hail(hailed, froms ['npc:<이름>'])`) |
...
| `npc_hails` | (2026-09-14 D71 additive, `run_meta.npc_hail` 판 · 있을 때만) 이 틱에 NPC 가 먼저 건 인사 `[{npc, char, line, key, line_src?, stopped?}]`(`stopped:true` = **2026-09-15 D76 additive**, 그 인사에 걸음을 멈추고 이 틱에 결정 — `run_meta.npc_hail_stop` 판) — key=고른 상황(hail/hail_no_potion/hail_board/hail_return/hail_rumor/hail_oracle). 같은 틱 `inbox[char]` 에 `from:'npc:<npc>'` 잡담으로 들어가 그 캐릭터의 이번 결정이 읽는다(걷는 중이면 보관됐다가 다음 결정) |
