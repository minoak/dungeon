...
| `sight` | 시야 반경(`DUNGEON_SIGHT`, 기본 5 — 2026-07-11 additive, 구판 스트림은 3). 봇 관측·봇 인지·몹 시야·목격이 전부 이 한 자(대칭) — **굴림 수를 바꾸는 세계 물리라 리플레이·판 비교는 seed 처럼 이 값까지 맞춰야 한다** |
| `scan` | 스캐너(D19) 여부(bool, `DUNGEON_SCAN`, 기본 false — 2026-07-12 additive, 암 B 판정 전 실험 스위치). true 면 ①obs.zone 이 구조 조회로 확장(`{id,kind, size/at(방), len/ends(통로), checked, doors[], exit?}` — 전부 방위·거리·딱지뿐, **좌표 없음**. id 는 기하 구역 `r<n>`/`c<n>` — level.rooms 와 조인 금지) ②`sights.traps[]`(드러난 함정 시야 어휘) ③문 id(`d<n>`)가 goto 핑·옵션에 등장 ④**걸음 정지 물리가 달라진다**(walk `entered`/`sighted` — 아래) ⑤장부 주소도 기하 구역 명의. 리플레이·판 비교는 이 필드까지 맞춰야 한다(ledger 와 같은 급) |
