...
| `npc_hails` | (2026-09-14 D71 additive, `run_meta.npc_hail` 판 · 있을 때만) 이 틱에 NPC 가 먼저 건 인사 `[{npc, char, line, key, line_src?}]` — key=고른 상황(hail/hail_no_potion/hail_board/hail_return/hail_rumor/hail_oracle). 같은 틱 `inbox[char]` 에 `from:'npc:<npc>'` 잡담으로 들어가 그 캐릭터의 이번 결정이 읽는다(걷는 중이면 보관됐다가 다음 결정) |
