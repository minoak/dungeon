...
| `events` | (2026-07-20 D22 additive) 사건층 여부(bool, `DUNGEON_EVENTS` 러너 기본 1·엔진 기본 0). true면 obs 에 목격 어휘가 늘고(witnessed: ally_hit/kill/trap/heal + 비몬스터 ally_down) 목격한 전사가 memories(fallen, 휘발 0)로 재제시된다 — obs 를 바꾸는 실행모드 메타(스트림 이벤트 자체는 불변, `grave` 병기 제외) |
...
| `monster_attack` | `id monster target roll mod total ac hit`, `surprise? from_hiding? dmg? hp? down? grave?` | 몹의 공격. surprise=몹 기습(they-ambush), from_hiding=매복자가 정체 드러내는 일격. **hit 이면 피격 인터럽트**(D1 개정): 당한 봇의 진행 중 order 가 그 자리에서 비워진다(다음 틱 스냅샷 `order:null` + 그 봇 재결정으로 관측 가능). 피격은 시드 RNG 결정론이므로 리플레이 무해. `grave`(2026-07-20 D22 additive, `DUNGEON_GRAVES` 판만) = down 과 함께 `{id,name,x,y}` — 쓰러진 자리에 선 '~의 묘' 피처(글리프 `T`). trap/chest_trap/fountain_harm 의 down 에도 같은 문법으로 병기 |
