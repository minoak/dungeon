...
| `broadcast_quiet` | (2026-09-08 D46 additive) 방송 무정지 여부(bool, `DUNGEON_BROADCAST_QUIET` 러너 기본 1). true 면 `to: all` 인 말은 들리기만 하고 **아무도 세우지 않는다**(지목 `to: <번호>` 만 세움 — D41 의 논리를 끝까지). 대화 뼈(D36)는 그대로 all 도 센다. 정지 물리 메타(sayto·hail 과 같은 급 — 리플레이·판 비교의 전제). 근거: 09-08 판 335835 에서 같은 선택 재발행의 73%·걸음 0 재결정의 88%가 말 걸림, 말의 54%가 all |
| `pending` | (2026-09-08 D46 additive) 들은 말 보관 여부(bool, `DUNGEON_PENDING` 러너 기본 1). true 면 걷는 동안 들린(안 세운) 말을 그 봇의 **다음 결정까지 보관**해 `tick.inbox` 에 함께 싣는다(상한 6, 오래된 것부터 바램) — 옛 판은 결정 없는 틱의 말이 증발했다. 작정 집행 틱(src=plan)은 view() 를 안 불러 못 읽으므로 보관 유지. `inbox[].turn`(말한 틱) 이 같은 날 additive 로 붙어 보관된 말의 나이를 안다 |
...
|---|---|
| `turn` | 틱 번호 |
| `inbox` | 이번 틱 **사고에 주입된** 받은편지함 `{char: [{from,text,to?},…]}` — 현재 파티 전원이 키(빈 리스트 포함). `to?`(2026-09-06 D41 additive)=그 말의 상대(봇 번호 | `all`, 발화 결정의 `to` 그대로 — 지목 판에선 이것으로 정지·대화 뼈를 가른다). 지난 틱 say 중 시야/근접 조건을 통과해 배달된 것. **층 전이(descend) 틱의 say 는 배달되지 않는다**(새 층에서 리셋) |
