...
| `scan` | 스캐너(D19) 여부(bool, `DUNGEON_SCAN`, 기본 false — 2026-07-12 additive, 암 B 판정 전 실험 스위치). true 면 ①obs.zone 이 구조 조회로 확장(`{id,kind, size/at(방), len/ends(통로), checked, doors[]}` — 전부 방위·거리·딱지뿐, **좌표 없음**. id 는 기하 구역 `r<n>`/`c<n>` — level.rooms 와 조인 금지. **계단은 구조에 없다** — 내용물이라 광학(sights.exit)으로만, 2026-07-12 정정 "시야 엔진은 말 그대로 시야 범위") ②`sights.traps[]`(드러난 함정 시야 어휘) ③문 id(`d<n>`)가 goto 핑·옵션에 등장 ④**걸음 정지 물리가 달라진다**(walk `entered`/`sighted` — 아래) ⑤장부 주소도 기하 구역 명의. 리플레이·판 비교는 이 필드까지 맞춰야 한다(ledger 와 같은 급) |
