...
- 목격(`witnessed[].kind`) `ally_zone{char, zone, zone_id}`(D87 — 구역 시야 판의 마을에서만): 나와 엮인 적 있는 사람(관계 장부에 뼈나 한 줄이 있는 사람)이 다른 구역으로 넘어가는 걸음 한 번에 한 줄 — 나가는 걸 본 사람과 들어오는 걸 본 사람에게. 관계 장부의 뼈 종류 `party`(파티를 맺음 — 파티 장부 판에서만, 새로 파티원이 된 쌍마다 한 번).
...
- `use`의 사물 기능 결과에는 `effect_type:interact|goto`가 붙어 기존 결과 어휘를 재사용한다. 물약 사용은 `result:healed`, `heal/hp/potions/item_used`; 축복의 물약(`i4`, **2026-09-15 D74 additive**) 사용은 `result:drink_boon`, `stat(str|dex)/value/boons/item_used`(빈 손이면 `nothing`); 효과 없는 조합은 `result:no_effect`, `reason_code`다. 공격 대상이 사람인 경우 `target_kind:bot`, `target_id:b<char>`이며 `monster_hp`는 구 소비자를 위한 호환 HP 필드다.
...
| `quests` | (2026-09-14 D69 additive) 길드 척추 여부(bool, `DUNGEON_QUESTS` 러너 기본 1 — notices 가 켜진 판만 뜻이 있다). true 면 ①마을 게시판 문턱 근처의 의뢰가 관측 대상 `q<n>`(notices.board.quests[].tid, 조합형 targets kind 'quest', 메뉴 '의뢰 맡기')이 되고 `use/interact q<n>` = 맡기(`interact` 결과 `quest_accepted{quest,title,goal,reward?,client?,board}` / `quest_already{quest,title,by,since}` / `too_far`) — 장부는 **파티 단위·판 전체**(`end.quests`) ②엔진이 완료를 센다: 처치(`attack` 결과 `quest[]` — `_damage_monster` 공용 지점이라 스킬 처치도), 층 도달(`level.quests[]`), 획득(`interact` treasure/chest_loot 의 `quest[]`) — 각 항목 `{id,title,n,need,done}` ③obs `quests[]`(맡은 의뢰·진행)·notices 항목 `accepted/done` ④**워프 귀환이 판을 끝내지 않는다** — 마을에서 이어 놀고(obs `expedition_returned`), 보고 역할 NPC(접수원, 정의 `npc.report`)에게 말을 걸면 `npc_report{npc,line,done[],undone[],titles{}}` = 원정의 끝(`end.outcome returned`, `end.turn` = 보고 틱). 안 하면 timeout(`end.warped` 로 구분). false/옛 판 = 게시판 정보만·워프 도착=종료(D65 그대로). 판 모양(종료 조건)을 바꾸는 실행모드 메타(boss 급) |
...
| `town_guide` | (2026-09-17 D81 additive) 마을 안내 여부(bool, `DUNGEON_TOWN_GUIDE` 러너 기본 1, 마을 판만). true 면 원정을 시작한 마을에서 각 캐릭터의 **첫 관측**에 `obs.town_guide {places[{name, zone, about}], allies[{char, zone}]}` 가 한 번 실리고 프롬프트에 '마을 안내' 문단(장소마다 정의의 특징 한 줄 + 동료의 위치)이 붙는다. 귀환 마을·이어가기엔 없다. 판정 무접촉 — 표현층 메타 |
...
| `npc_hail_stop` | (2026-09-15 D76 additive) NPC 인사에 걸음을 멈추는 판(bool, `DUNGEON_NPC_HAIL_STOP` 러너 기본 1 — npc_hail 판만). true 면 인사를 받은 걷던·기다리던 캐릭터의 order 가 끊기고 그 틱에 결정한다(`tick.npc_hails[].stopped`, 직전 결과 `hail(hailed, froms ['npc:<이름>'])`) · **2026-09-16 개정**: 곁에 닿아 다음 틱에 거래·공격을 실행할 참인 접근(arrived/ready)은 세우지 않는다(`stopped` 없음) — 인사는 보관돼(D47 배관) 거래 뒤 첫 결정에서 읽힌다 |
...
|---|---|
| `turn` | 틱 번호 |
| `inbox` | 이번 틱 **사고에 주입된** 받은편지함 `{char: [{from,text,turn,to?,kind?},…]}` — 현재 파티 전원이 키(빈 리스트 포함). `from` 이 `npc:<이름>`(2026-09-14 D69 additive)이면 마을 NPC 의 답(잡담·정지 없음·관계 뼈 없음, `to`=말 건 봇). `to?`(2026-09-06 D41 additive)=그 말의 상대(봇 번호 | `all`, 발화 결정의 `to` 그대로 — 지목 판에선 이것으로 정지·대화 뼈를 가른다). 지난 틱 say 중 시야/근접 조건을 통과해 배달된 것. **층 전이(descend) 틱의 say 는 배달되지 않는다**(새 층에서 리셋) |
| `npc_hails` | (2026-09-14 D71 additive, `run_meta.npc_hail` 판 · 있을 때만) 이 틱에 NPC 가 먼저 건 인사 `[{npc, char, line, key, line_src?, stopped?}]`(`stopped:true` = **2026-09-15 D76 additive**, 그 인사에 걸음을 멈추고 이 틱에 결정 — `run_meta.npc_hail_stop` 판 · 곁에 닿아 거래 직전인 봇은 서지 않는다 = 2026-09-16 개정) — key=고른 상황(hail/hail_no_potion/hail_board/hail_return/hail_rumor/hail_oracle). 같은 틱 `inbox[char]` 에 `from:'npc:<npc>'` 잡담으로 들어가 그 캐릭터의 이번 결정이 읽는다(걷는 중이면 보관됐다가 다음 결정) |
...
| `turn` | 전이가 일어난 틱 |
| `to_depth` | 내려가는 층 번호 |
| `party[]` | 생존 이월자 `{char, hp, bag, potions, boons?}` (char 순 정렬. potions=2026-07-17 additive — 물약도 들고 내려간다. boons=2026-09-15 D74 additive, 있을 때만 — 축복으로 오른 str/dex 도 재스폰이 그대로 실어 나른다. 장비는 여기 없음 — 이월 상태는 다음 level.party/tick.bots 의 weapon/armor 로 확인) |
...
|---|---|
| `turn` | 종료 틱 |
| `outcome` | `escaped`(최심층 돌파·탈출) / `wiped`(전멸) / `timeout`(틱 한도) / `returned`(2026-09-13 D65 — 보스를 잡고 워프게이트로 마을 귀환, 원정 완료. **2026-09-14 D69**: `run_meta.quests` 판에선 귀환 뒤 길드 접수원에게 보고한 틱 = 종료 — 보고 없이 틱 한도면 `timeout` + `warped`) — 러너 종료 3분기와 1:1 |
| `quests` | (2026-09-14 D69 additive, `run_meta.quests` 판만) 의뢰 장부 `{accepted[{id,turn,by}], done{id:turn}, progress{id:n}, returned(워프 귀환 틱|null), reported(보고 틱|null)}` — 파티 단위. 같은 판의 `quest_accepted`·`quest[]`·`level.quests`·`npc_report` 사건에서 파생 가능 |
| `warped` | (2026-09-14 D69 additive) `true` = 워프게이트로 마을에 돌아온 판(보고 여부와 무관 — timeout 이어도 돌아오긴 했다) |
| `depth` | 종료 시 층 |
...
- **봇**: `char job sex x y hp maxhp bag potions weapon armor alive won order aware_of[]` · `boons?`(**2026-09-15 D74 additive**, 축복의 물약 병 수 — 있을 때만)
...
| `interact` | `target`, `result=exit(party[])/wait_allies(missing[])/treasure/potion(potions — 2026-07-17 additive)/chest_loot(roll,mod,total,loot)/chest_trap(roll,mod,total,dmg,hp,down?,status? — 2026-09-06 D34: 생존 시 중독)/fountain_heal(roll,heal,hp)/fountain_harm(roll,dmg,hp,down?,status? — 2026-09-06 D34: 생존 시 중독)/equip(item,slot,bonus,dropped? — 2026-07-30 D28 additive)/ascend(party[] — 마을 복귀, D29 additive)/npc_talk(npc,line,again? — 마을 NPC 말 걸기, D29 additive. `again:true` = 2026-09-06 D32 개정 additive: 같은 방문에서 두 번째 이후 말 걸기(방문 장부 `npc_met`, 선물 여부와 무관) — 이때 line 은 town.json `line_again`("아까 왔잖아. …") 고정 대사)/npc_gift(npc,item,line — 마을 상점 v0, D32 additive 2026-09-05: 아이템 상인=물약 1/방문·장비 상인=빈손이면 단검, 정해진 대사만. **2026-09-15 D74**: 성직자=축복의 물약(`gift.boon`, item '축복의 물약', 방문당 한 병). 받는 장면은 동료 witnessed 에 ally_loot)/nothing/too_far/no_target` · **2026-09-14 D69 additive**(`run_meta.quests` 판): `quest_accepted(quest,title,goal,reward?,client?,board — target 'q<n>')` / `quest_already(quest,title,by,since)` / `npc_report(npc,line,done[],undone[],titles{},bag — 귀환 뒤 보고 역할 NPC, 원정의 끝)` · treasure/chest_loot 에 `quest?[]`(획득형 의뢰 진행) · npc_talk/npc_gift/npc_report 에 `line_src?:'brain'`+`line_fixed?`(NPC 두뇌가 line 을 썼다 — 원문은 line_fixed) | 계단(파티 동반 하강/대기)·줍기·상자·샘·물약 집기·장비 착용(스왑 시 dropped=그 자리에 놓은 헌 장비 이름 — 같은 칸에 새 피처로 실린다). exit 의 party=함께 내려간 char 명단 |
| `attack` | `result=attack/no_target/too_far`(**항상 존재**). `attack` 일 때만 `target`(몹 종류)·`target_id`(`m<n>`)·`roll mod total ac hit`·`surprise? crit? dmg? monster_hp? killed?` 존재 — 실패 2종은 `char/type/result` 뿐. `quest?[]`(2026-09-14 D69 additive) = 이 처치가 채운 의뢰 `{id,title,n,need,done}`(스킬 처치의 skill 결과에도 같은 자리) | 봇의 공격. **result 로 분기하라.** surprise=우리 기습(we-ambush) |
| `search` | `radius`, `found[]` | 능동 수색(턴 소모, 반경 내 확정 발견). found 빈 배열=허탕 |
| `drink` | `result=drink_heal(heal,hp,potions)/no_potion` · **2026-09-15 D74 additive**(메뉴형 `item:boon`): `drink_boon(stat,value,boons)/no_boon` — 축복의 물약, 공격 능력치(근접=str·원거리=dex) +1 영구(이 판), 병 −1 | (2026-07-17 additive) 회복 물약 마시기 — 굴림 없는 확정 완전 회복(샘=도박과 대비되는 '들고 다니는 보험'), 한 턴 소모. `potions`=남은 병 수. `no_potion`=빈 손 정직 보고. 만피에 마셔도 소모(heal=0 — 세계는 낭비를 말리지 않는다) |
| `give` | `target(b<char>)`, `item(potion|boon|weapon|armor)`, `result=given(to,what,potions?|boons?|equipped?|placed?)/too_far/no_target/nothing/no_room` | (2026-09-09 D47 ② additive, `run_meta.give` 판) 건네기 — 곁의 동료에게 소지품을 넘긴다(굴림 없음·한 턴). `what`=물약|장비 이름. potions=남은 병, equipped=상대가 바로 걸침, placed=상대 슬롯이 차 있어 발밑(상대 칸, 찼으면 내 칸)에 피처로 놓임 → 같은 틱 features 에 새 피처. 받은 쪽은 이벤트가 아니라 그 봇의 다음 obs.last/trail(`type:received{from,what,item,…}`)로 보고(걷는 중이면 안 선다, 대기·휴식이면 깬다). 둘을 뺀 시야 안 동료 witnessed 에 `ally_give{char,to,what}`. 관계 뼈 gave/received |
