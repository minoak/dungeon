...
MAPS = {                                          # 시작 옵션 → 러너 환경변수(wonderland.bat 메뉴 값 그대로)
    ...
    # D88(09-20) 새 던전 생성 프로필 — 넓은 통로·대홀·기둥의 석조 던전. 크기를 여기서 같이 준다(부모 env 의
    # DUNGEON_W/H 를 덮는다). ⚠️키 이름은 바꾸지 않는다 — 멈춰 둔 판의 run_opts.json 이 이 이름으로 이어간다(D79).
    # 09-20 낮 개정(민옥 "차라리 던전을 조금 더 크게 하고 층을 3층으로 놓는것도 괜찮아 보이네"): 42×34 → 54×42.
    #   층을 3층으로 줄이는 대신 한 층을 넓힌다. 방 수는 생성기가 넓이비로 같이 늘린다(dungeon_concept.ROOM_TARGET_*)
    #   — 격자만 키우면 방 수가 그대로라 빈 암반과 긴 통로만 늘기 때문.
    #   0콜 실측 120시드(verify_arch ③-2 가 같은 수를 찍는다): 방 6.37 → 10.45개 · 바닥 460 → 717칸 ·
    #   한 구역이 바닥을 먹는 비율 평균 0.282 → 0.176(최대 0.635 → 0.330) = 넓혀도 구역이 덜 뭉친다.
    #   1800층 스윕(깊이 1~3 × 시드 600)에서 예외·끊긴 바닥·못 닿는 출구·보스 없는 보스층 0.
    #   몹 수 4(파트너 "지금은 몹수는 4으로 늘리자"). 그 값을 정할 때의 측정은 옛 5층 세계(42×34·600틱)의 것이었다 —
    #     09-20 밤 리뷰에서 3층 세계로 다시 쟀고, 아래가 그 수다.
    #   ⚠️DUNGEON_MONSTERS 는 '그 층에 놓는 기본 몹 수'일 뿐 층에 실제로 서는 수가 아니다 — 잠복 1마리(DUNGEON_LURKERS)·
    #     보스층의 보스·D92 새 몬스터의 무리 확장이 그 위에 얹힌다. 0콜 실측(40시드 · 54×42 · 3층 · 새 몬스터 켬):
    #       몹 2 → 층별 3 / 5.1 / 7.5 = 3층 합계 15.5마리
    #       몹 4 → 층별 5 / 8 / 10    = 3층 합계 23.0마리
    #   0콜 난이도 실측(더미 두뇌·진짜 몹 수치·시드 10·마을 끔·3층·1200틱 = 그때의 상한. ⚠️'마을에 약 245틱' 은 09-20 저녁에
    #     약 410틱으로 정정됐다 — 이 난이도 수치 자체는 마을을 끄고 잰 것이라 그대로 유효하다):
    #       몹 2 → 전멸 5 · 귀환 4 · 틱상한 1 (3층 도달 10/10)
    #       몹 4 → 전멸 9 · 귀환 1 · 틱상한 0 (3층 도달  7/10)
    #     ⚠️규칙 두뇌는 도망도 물약도 제대로 못 쓴다 — 이 수치는 하한이다. 다만 같은 두뇌·같은 세계에서 2 와 4 가 갈린다
    #       (옛 5층 세계의 측정은 둘을 못 갈랐다: 몹 2 도 7/10 이 전멸했다).
    #     실LLM 판(runs/stream-20260920-091028.jsonl · monsters=2 · 42×34)의 층별 실제 몹은 3 / 6 / 7 이었고 아무도 죽지 않았다.
    #   ⚠️값 임시 — 한 낱말로 바꾼다(2 = 합계 15.5마리 · 4 = 합계 23마리). 민옥 확인 대기(09-20 밤 리뷰 발견).
    "concept": {"DUNGEON_W": "54", "DUNGEON_H": "42", "DUNGEON_ARCH": "concept", "DUNGEON_MONSTERS": "4"},
}
...
# 09-20 저녁 개정 1200 → 1800(파트너 "어찌됬든 600턴을 넘길수 있어야 하는거 아냐?"): 아래 Runner.start 의 긴 주석 참고.
#   한 줄 요약 — 마을 몫을 245틱으로 잡고 뽑은 1200 이었는데 실측이 410틱이었다.
#   ⚠️09-20 저녁 재측정: 콜/틱은 하나의 수가 아니다 — 마을 0.33 · 1층 1.07(던전이 3배 넘게 비싸다).
...
class Runner:
    ...
    def start(self, opts, party_path, default_brain=None, extra_env=None):
        ...
        with self.lock:
            ...
            # 다른 맵을 고른 판은 옛 생성기로 지어야 한다(새 생성기가 검증된 격자는 26x17~72x54 — 56x20·80x30 은 세로가 검증 밖).
            if "DUNGEON_ARCH" not in MAPS[m]:
                env.pop("DUNGEON_ARCH", None)
            # 09-20 낮 개정(민옥 "이전부터 600턴 이내에 5층 돌파는 좀 무리가 컸어 … 차라리 던전을 조금 더 크게 하고 층을
            #   3층으로 놓는것도 괜찮아 보이네"): 5층 600틱 → 3층 1200틱. 층을 줄이고 한 층을 넓힌다(MAPS['concept'] 54×42).
            # 근거 — 이 리포에 남은 단 하나의 실LLM concept 판(시드 482822 · 42×34 · 5층 · 600틱 상한,
            #   runs/stream-20260920-091028.jsonl 의 level 줄): 마을 245틱 · 1층 211틱 · 2층 100틱 ·
            #   3층은 44틱 만에 틱 상한(outcome timeout — 보스는 구경도 못 했다).
            #   한 층 평균 156틱 × 넓어진 배율 1.56(바닥 460 → 717칸)= 약 243틱. 마을 245 + 243×3 = 약 974.
            #   09-20 밤 수선(리뷰 발견): 974 는 '3층 바닥을 밟은 순간'까지다 — 거기서 판이 끝나면 보스를 못 본다.
            #   보스층 뒷자락(보스전·상자·워프)은 따로 재야 한다. 리포에 남은 단 둘뿐인 outcome=returned 판
            #   (runs/stream-20260913-175634.jsonl · -192230.jsonl — D67 '보스 앞에서 시작' 프리셋이라 층 탐색도 마을도 없다):
            #   보스 앞 → 워프 귀환까지 51틱과 161틱(평균 106). 워프 뒤 접수원 보고(D94)까지 마을을 다시 건너는 몫도 있다.
            #   974 + 106 + 약 100 = 약 1,180 → 1200. ⚠️값 임시 — 실LLM 3층 완주 판으로 다시 재야 한다.
            #   (마을은 안 넓어졌으므로 245 를 그대로 둔다. 표본이 한 판뿐이라 층당 틱의 오차는 크다.)
            # ── 09-20 저녁 개정: 1200 → 1800. 위 계산의 '마을 245' 가 틀렸다.
            #   파트너 "어찌됬든 600턴을 넘길수 있어야 하는거 아냐?" 에서 출발해 기록 157판을 전수로 다시 쟀다(0콜):
            #     · 600틱을 넘긴 판은 한 판도 없었다 — 전부 상한이 600 이었기 때문이지 벽이 아니다.
            #       (show_runner.py:116 의 600 은 DUNGEON_TURNS 가 없을 때의 기본값이고, 루프는 range(_, MAX_TURNS+1) 이다.
            #        0콜 1500틱 판이 outcome=timeout 으로 끝났고 층 전이가 0→1→2→3→0→1→2→3 = D94 루프 두 바퀴였다.)
            #     · 마을을 떠난 틱: 옛 판 116개는 중앙값 17 이었는데, 09-19 무렵 225, 마을생활을 켠 08:42 판이 245,
            #       그리고 runs/stream-20260920-134954.jsonl 은 405틱까지 아예 안 내려갔다.
            #   그 405틱 판의 결정을 읽어 보면 막힌 것이 아니었다 — 'goto exit' 를 21번 냈고 남긴 이유가 한결같다:
            #     "던전에 들어가려면 3인 파티가 필요하다"(t49) · "3인 파티만 입장 가능"(t201) · "마침내 3인 파티가 결성되었다"(t344).
            #     파티를 맞추는 데 344틱을 쓰고, 그 뒤에도 구역 시야(D86)로 서로 놓쳐 쫓느라 60틱을 더 썼다.
            #     그 판의 마지막 틱에서 두란은 (69,53), 던전 입구는 (77,53) — 여덟 칸 앞에서 사람 손에 멈춘 판이다.
            #   그래서 마을 몫을 245 가 아니라 약 410 으로 본다: 410 + 729 + 206 = 약 1,345 > 1200.
            #   ⚠️'함께 출발'(TOWN_APART=0, 서버 기본)이 이 중 파티 결성 몫을 크게 줄인다 — 그래도 245 로 돌아가지는 않는다.
            #   1800 을 고른 이유는 여유가 아니라 두 한도를 같은 자리에 두기 위해서다: 1800 × 0.66콜 ≈ 1,190콜 ≈ 서버 호출 한도 1200.
            #   ⚠️09-20 저녁 재측정으로 그 0.66 자체가 못 믿을 수가 됐다(아래 '콜/틱은 하나의 수가 아니다' 절).
            #   지금까지는 호출 예산이 남았는데 틱이 먼저 판을 잘랐다. ⚠️여전히 실LLM 3층 완주 판으로 재야 하는 값이다.
            # ⚠️공개 서버는 판마다 두뇌 호출 한도가 있다(DUNGEON_API_CALL_LIMIT — 유닛 템플릿 scripts/vm/botpikdun.service 는 1200).
            #   같은 판의 실측 호출 수는 결정 375 + NPC 되받기 19 = 394/600틱 = 틱당 약 0.66(사회 반응 255건은 같은 응답에서
            #   읽는 것이라 호출이 아니다). 1,200틱 판이면 약 790콜, 1,800틱 판이면 약 1,190콜이다.
            # ── ⚠️콜/틱은 하나의 수가 아니다(09-20 저녁 재측정, 실판 220틱 관측 — 민옥의 데모 판 1800틱·함께 출발·loop):
            #       마을 190틱 · 결정  63건 = 0.33 결정/틱   (엔진 자동보행이 대부분이라 결정이 드물다)
            #       1층   30틱 · 결정  32건 = 1.07 결정/틱   (던전이 3배 넘게 비싸다)
            #   위의 0.66 은 마을이 길고 던전이 짧았던 600틱 판의 혼합 평균이라, 혼합비가 바뀌면 무너진다.
            #   마을 191틱 뒤 끝까지 던전이면 63 + 1609×1.07 ≈ 1,780콜 — 틱 상한 1800 을 다 쓰려면 호출 한도도 그쯤이어야 한다.
            #   호출 한도 1200 이면 대략 t1256 에서 먼저 닿는다(완주 추정 t1126 의 누적은 약 1,061콜 = 여유 12%).
            #   ⚠️위 1.07 은 그 판의 앞 30틱만 본 수라 틀렸다. 같은 판이 끝까지 가서 잰 값(첫 실 LLM 완주, 09-20 저녁):
            #       1차 원정 878틱 · 438콜 = 0.499콜/틱
            #       (마을 0.408 / 1층 0.636 / 2층 0.531 / 3층+보스+귀환 0.529 / 마을 보고 0.104)
            #   한도 1200 이면 완주에 438콜을 쓰고 762콜(64%)이 남는다 — 2차 원정까지 돌아도 안 넘는다.
            #   ⚠️교훈: 콜/틱을 짧은 창으로 외삽하지 말 것(이 주석 안에서만 0.66·1.07 두 번 틀렸다).
            #   ⚠️틱 상한과 호출 한도는 다른 축이다: 한도가 먼저 닿으면 판은 그 자리에서 멈추고, 방문자가 제 열쇠로 이어간다
            #   (민옥 결정 09-20 "서버의 처음 화면에서 api키로 이어서 플레이" — 그 흐름은 서버 쪽이 맡는다).
