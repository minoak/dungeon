...
MAPS = {                                          # 시작 옵션 → 러너 환경변수(wonderland.bat 메뉴 값 그대로)
    ...
    #   1800층 스윕(깊이 1~3 × 시드 600)에서 예외·끊긴 바닥·못 닿는 출구·보스 없는 보스층 0.
        #   몹 수 4(파트너 "지금은 몹수는 4으로 늘리자"). 0콜 실측(더미 두뇌·시드 10·600틱·보스 실수치):
    ...
    #   ⚠️값 임시 — 험하면 DUNGEON_MONSTERS 한 낱말만 고친다(2 로 되돌리면 옛 몹 수).
...
class Runner:
    ...
    def start(self, opts, party_path, default_brain=None, extra_env=None):
        ...
        with self.lock:
            ...
            #   3층으로 놓는것도 괜찮아 보이네"): 5층 600틱 → 3층 1000틱. 층을 줄이고 한 층을 넓힌다(MAPS['concept'] 54×42).
            ...
            #   → 보스층은 싸움이 붙어 더 길므로 1000. ⚠️값 임시 — 실LLM 3층 완주 판으로 다시 재야 한다.
            #   (마을은 안 넓어졌으므로 245 를 그대로 둔다. 표본이 한 판뿐이라 층당 틱의 오차는 크다.)
            # ⚠️공개 서버는 판마다 두뇌 호출 한도가 있다(DUNGEON_API_CALL_LIMIT). 같은 판의 실측 호출 수는
            #   결정 375 + NPC 되받기 19 = 394/600틱 = 틱당 약 0.66(사회 반응 255건은 같은 응답에서 읽는 것이라 호출이 아니다).
            #   한도 500이면 약 750틱에서 판이 멈춘다 — 1000틱을 다 쓰는 것은 한도가 없는 로컬·BYOK 판이다.
            #   한도에 닿아 멈춘 판을 열쇠로 이어가는 것이 정식 흐름이다(민옥 결정 09-20) — 그 흐름은 서버 쪽이 맡는다.
            if mode == "standard":
                env.update(DUNGEON_DEPTHS="3", DUNGEON_TURNS="1000", DUNGEON_SOLO="0")
            if opts.get("town"):
                env["DUNGEON_TOWN"] = "1"
            else:
